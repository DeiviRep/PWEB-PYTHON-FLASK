...
def add_contact():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute('INSERT INTO user(usuario,email,contraseña) VALUES(%s,%s,aes_encrypt(%s,"toto"));',(usuario,email,password))
...
def get_contact(id):
    cur = mysql.connection.cursor()
    cur.execute('SELECT * FROM user WHERE id = {0};'.format(id))
    data = cur.fetchall()
